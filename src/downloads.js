...
const INPROGRESS_ROOT = path.join(
  path.dirname(__dirname),
  'data/download_inprogress/'
);
  ...
  switch (task_name) {
    ...
    case 'matchlog':
      return (
        ...
        '.aspx' +
        '?id=' +
        encodeURIComponent(tournament_id) +
        '&ft=1&sd=20000101000000&ed=20990101000000'
      );
    case 'clubranking':
      return (
        BASE_URL +
...
function run_login(config, jar, cb) {
  ...
  request.get(
    ...
    function(err, _, html) {
      ...
      var m = /<form\s+method="post"\s+action="(\.\/login[^"]*)"([\s\S]*?)<\/form>/.exec(
        html
      );
      if (!m) {
      ...
      var input_vals = utils.match_all(
        /<input\s+(?:[a-z0-9-]+="[^"]*"\s+)*?name="([^"]+)"\s+(?:[a-z0-9-]+="[^"]*"\s+)*?value="([^"]*)"/g,
        form_html
      );
      var form_data = {};
      ...
      form_data[
        'ctl00$ctl00$ctl00$cphPage$cphPage$cphPage$pnlLogin$UserName'
      ] = config('tournament_user');
      form_data[
        'ctl00$ctl00$ctl00$cphPage$cphPage$cphPage$pnlLogin$Password'
      ] = config('tournament_password');
  ...
  download_file(req, fn, err => {
    ...
    fs.stat(fn, (err, stats) => {
      ...
      if (stats.size === 0) {
        log_verbose(
          config,
          'Download ' + season.name + ' / ' + task_name + ' is empty'
        );
      ...
      log_verbose(
        config,
        'Finished downloading ' + season.name + ' / ' + task_name
      );
      ...
      function(cb) {
        ...
        utils.ensure_dir(download_dir, function(err) {
          ...
          if (season.buli_tournament_id) {
            ...
            const use_buli_tasks = all_buli_tasks.filter(
              t => t !== 'clubranking'
...
function download_job(app, season, cb_started, cb_finished) {
  download_season(
    ...
    function(err, dl) {
      ...
      fs.rename(
        path.join(INPROGRESS_ROOT, dl.id),
        path.join(DATA_ROOT, dl.id),
        function(err) {
          dl.done_timestamp = Date.now();
          if (err) {
            dl.status = 'error';
