...
function receiver_add_handler(req, res, next) {
  ...
  req.app.db.seasons.update(
    { key: req.params.season_key },
    ...
    function(err) {
      ...
      res.redirect(
        req.app.root_path +
          's/' +
          encodeURIComponent(req.params.season_key) +
          '/'
      );
    }
  );
...
function ignore_handler(req, res, next) {
  req.app.db.seasons.update(
    { key: req.params.season_key },
    { $addToSet: { ignore: req.body.problem_id } },
...
function unignore_handler(req, res, next) {
  req.app.db.seasons.update(
    { key: req.params.season_key },
    { $pull: { ignore: req.body.problem_id } },
    {},
    function(err) {
      if (err) return next(err);
      res.redirect(
...
function recheck_handler(req, res, next) {
  check.recheck(
    ...
    req.app.config('check_background'),
    function(err) {
      if (err) return next(err);
      res.redirect(
        req.app.root_path +
