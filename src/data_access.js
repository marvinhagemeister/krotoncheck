...
function annotate(data) {
  ...
  for (const pm of data.playermatches) {
    ...
    for (const bool_key of [
      'flag_keinspiel_keinespieler',
      'flag_keinspiel_keinspieler_team1',
      'flag_keinspiel_keinspieler_team2',
      'flag_aufgabe_team1',
      'flag_aufgabe_team2',
      'flag_umwertung_gegen_team1',
      'flag_umwertung_gegen_team2',
...
function enrich(season) {
  ...
  const buli_player_by_id = buli_make_index(
    data.buli_players,
    p => p.spielerid
  );
  ...
  const buli_teammatch_by_id = buli_make_index(
    data.buli_teammatches,
    tm => tm.matchid
  );
  const matchlogs_by_teammatchid = utils.make_multi_index(
    data.matchlog,
    ml => ml.matchid
  );
  const playermatches_by_teammatchid = utils.make_multi_index(
    data.playermatches,
    pm => pm.teammatchid
  );
  ...
  const matchcomments_by_tmid = utils.make_multi_index(
    data.matchcomments,
    line => line.matchid
  );
  ...
  for (let tm of data.teammatches) {
    ...
    if (!t1) {
      throw new Error(
        `Team1 (ID: ${tm.team1id}) in teammatch ${tm.matchid} is missing`
      );
    }
    let t2 = team_by_id.get(tm.team2id);
    if (!t2) {
      throw new Error(
        `Team2 (ID: ${tm.team2id})  in teammatch ${tm.matchid} is missing`
      );
    }
  ...
  if (data.spielgemeinschaften) {
    ...
    data.in_sg = (clubcode1, clubcode2) => {
      return (
        sg_pairs.has(clubcode1 + '_' + clubcode2) ||
        sg_pairs.has(clubcode2 + '_' + clubcode1)
      );
    };
  }

  data.get_player = (player_id, include_buli) => {
    ...
    if (!res) {
      throw new Error(
        'Konnte Spieler ' + JSON.stringify(player_id) + ' nicht finden'
      );
    }
    return res;
  };
  data.get_teammatch = teammatch_id => {
    const res = teammatch_by_id.get(teammatch_id);
    if (!res) {
      throw new Error(
        'Konnte Wettkampf ' + JSON.stringify(teammatch_id) + ' nicht finden'
      );
    }
    return res;
  };
  data.try_get_teammatch = teammatch_id => {
    return teammatch_by_id.get(teammatch_id);
  };
  data.get_teammatches_by_team_id = team_id => {
  ...
  data.get_match = match_id => {
    ...
    if (!res) {
      throw new Error(
        'Konnte Spiel ' + JSON.stringify(match_id) + ' nicht finden'
      );
    }
    return res;
  };
  data.get_playermatches_by_teammatch_id = teammatch_id => {
    const res = playermatches_by_teammatchid.get(teammatch_id);
    if (!res) {
      throw new Error(
        'Konnte Spiele von Wettkampf ' +
          JSON.stringify(teammatch_id) +
          ' nicht finden'
      );
    }
    return res;
  };
  data.try_get_team = team_id => {
    return team_by_id.get(team_id);
  };
  data.get_team = (team_id, include_buli) => {
  ...
  data.get_vrl_entry = (club_id, vrl_type, player_id) => {
  ...
  data.try_get_vrl_entry = (club_id, vrl_type, player_id, allow_no_club) => {
    let club_vrls = vrls_by_clubs.get(club_id);
    if (!club_vrls) {
      if (allow_no_club) {
        return null;
  ...
  data.get_teams_by_club = club_code => {
  ...
  data.try_get_teams_by_club = club_code => {
  ...
  data.get_vrl_entries = (clubcode, typeid) => {
  ...
  data.get_player_matches = (pcode, is_hr) => {
  ...
  data.get_all_notes = tm_id => {
    return matchcomments_by_tmid.get(tm_id);
  };
  data.get_stb_note = (tm_id, textfilter) => {
  ...
  data.get_comment = (tm_id, textfilter) => {
    const comments = matchcomments_by_tmid.get(tm_id);
    if (!comments) {
      return null;
    }
