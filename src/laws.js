...
function is_backup(registered_in, played_in) {
  ...
  if (!registered_m) {
    throw new Error(
      'Cannot parse registered_in event ' + registered_in.eventname
    );
  }
  ...
  if (played_m[1] === 'O19') {
    ...
    return (
      data_utils.parse_int(registered_in.number) >
      data_utils.parse_int(played_in.number)
    );
  }

  const [played_age_idx, played_team_idx] = _parse_youth_team(
    played_in,
    played_m
  );
  const [registered_age_idx, registered_team_idx] = _parse_youth_team(
    registered_in,
    registered_m
  );
  ...
  return (
    ...
    (registered_age_idx === played_age_idx &&
      registered_team_idx > played_team_idx)
  );
}
...
function forced_retreat(data, team_id) {
  ...
  for (const tm of tms) {
    if (
      (tm.flag_ok_gegen_team1 && tm.team1id === team_id) ||
      (tm.flag_ok_gegen_team2 && tm.team2id === team_id)
    ) {
      strikes++;
      if (strikes === 3) {
        return tm;
