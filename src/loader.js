  ...
  async.waterfall(
    [
      ...
      function(season, cb) {
        ...
        parse_csv(
          season.sg_csv,
          'Spielgemeinschaften-' + season.key + '.csv',
          function(err, sg_table) {
  ...
  Baby.parse(fcontents, {
    ...
    complete: function(res) {
      if (res.errors.length > 0) {
        return cb(
          new Error('Failed to parse ' + fn + ': ' + JSON.stringify(res.errors))
        );
      }
