...
function calc_bottom_msg(ar) {
  return `Automatisch verschickte E-Mail. Job-ID: ${ar.name}, schedule ${
    ar.schedule
  }.`;
}
...
function run(config, db, ar_id) {
  async.waterfall(
    [
      ...
      (ar, cb) => {
        db.seasons.findOne({ key: ar.season_key }, (err, season) => {
          if (err) return cb(err);
          if (!season)
            return cb(new Error('Cannot find season ' + ar.season_key));
          cb(err, ar, season);
        });
      },
      (ar, season, cb) => {
      ...
      (ar, season, dl, found, crafted, cb) => {
        kc_email.sendall(config, crafted, (err, errors) =>
          cb(err, ar, season, dl, found, crafted, errors)
        );
      },
      (ar, season, dl, found, crafted, errors, cb) => {
        db.autoruns.update(
          { _id: ar_id },
    ...
    err => {
      if (err) {
        console.error(
          `Autorun ${ar_id} failed: ${err.message} ${err.stack} (${err})`
        );
        return;
      }
    }
...
function create_handler(req, res, next) {
  ...
  req.app.db.autoruns.insert(arun, err => {
    if (err) return next(err);
    setup(req.app.config, req.app.db, arun, err => {
      if (err) return next(err);

      res.redirect(
        `${req.app.root_path}s/${encodeURIComponent(season_key)}
          /#autorun_${arun._id}`
      );
    });
  });
...
function receiver_add_handler(req, res, next) {
  ...
  req.app.db.autoruns.update(
    { _id: ar_id },
    { $addToSet: { receivers: receiver } },
    {},
    err => {
      if (err) return next(err);
      res.redirect(
        `${req.app.root_path}s/${encodeURIComponent(
          req.params.season_key
        )}/#autorun_${ar_id}`
      );
    }
  );
