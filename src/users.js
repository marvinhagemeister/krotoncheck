...
function login_handler(req, res, next) {
  ...
  req.app.db.users.findOne({ email: email }, function(err, u) {
    ...
    res.cookie('krotoncheck_session', session.key, {
...
function change_password_handler(req, res, next) {
  ...
  if (
    !bcrypt.compareSync(req.body.old_password, req.krotoncheck_user.password)
  ) {
...
function middleware(req, res, next) {
  ...
  get_user_by_session(req.app.db, req.cookies.krotoncheck_session, function(
    err,
    user
  ) {
    if (err) {
