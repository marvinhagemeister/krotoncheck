...
function filter_receiver(problems_struct, receiver) {
  ...
  res.found = problems_struct.found.filter(function(problem) {
    ...
    if (receiver.regions_filter) {
      ...
      if (
        !receiver.regions_filter.some(region => problem.region.includes(region))
      ) {
        return false;
      }
    }
...
function bw_receivers(season, callback) {
  ...
  for (let line of lines) {
    ...
    if (!emails_str) {
      console.error(
        'Invalid bw_receivers configuration: ' +
          JSON.stringify(line) +
          '. Skipping'
      );
      continue;
    }
    const regions_filter = regions_str
      .split('+')
      .map(s => s.trim())
      .filter(s => s);
    const emails = emails_str
      .split(' ')
      .map(s => s.trim())
      .filter(s => s);
...
function craft_emails(
  ...
  const tasks = [cb => cb(null, default_receivers)];
  if (add_receivers) {
...
function craft_single_email(
  ...
  render.render_standalone('mail_basic', data, function(err, body_html) {
    ...
    const receiver_class = receiver.stb_filter
      ? 'StB'
    ...
    render.render_standalone('mail_scaffold', res, function(err, mail_html) {
      ...
      render.render_standalone('plaintext_basic', data, function(
        err,
        rendered_mail_text
      ) {
        if (err) return cb(err);
