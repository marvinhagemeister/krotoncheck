    ...
    if (p.type === 'vrl' || p.type === 'fixed' || p.type === 'vrl_generic') {
      ...
      if (p.vrl_typeid) {
        p.header =
          ...
          club.name +
          ' ' +
          data.vrl_name(p.vrl_typeid);
      }
      if (p.type === 'fixed') {
        p.turnier_url =
          downloads.BASE_URL +
          'sport/teammatch.aspx?id=' +
          season.tournament_id +
          '&match=' +
          p.teammatch_id;
        p.turnier_vrl_url =
          downloads.BASE_URL +
          'sport/clubranking.aspx?id=' +
          season.tournament_id +
          '&cid=' +
          club.XTPID;
...
function prepare_render(season, problems) {
  ...
  problems.sort((f1, f2) => {
    ...
    if (f1.teammatch && f2.teammatch) {
      if (
        f1.teammatch.ergebnisbestaetigt_datum &&
        !f2.teammatch.ergebnisbestaetigt_datum
      ) {
        return -1;
      }
      if (
...
function color_render(problems_struct) {
  const problems = problems_struct
    ? problems_struct.found ? problems_struct.found : []
    : [];
  ...
  for (const problem of problems) {
    ...
    if (problem.type === 'vrl' || problem.type === 'vrl_generic') {
      ...
      if (!by_group) {
        by_group = {
          turnier_url:
            'mailto:phihag@phihag.de?subject=Interner+Fehler+in+krotoncheck',
          problems: [],
        };
  ...
  for (const region of utils.values(by_color)) {
    for (const groups of utils.values(region.regions_map)) {
      for (const g of utils.values(groups.groups_map)) {
              ...
              g.problems
                .map(p => {
                  ...
                  return (
                    ...
                    ' (' +
                    utils.weekday_destr(utils.parse_date(tm.spieldatum)) +
                    ', ' +
                    tm.spieldatum +
                    ')\n' +
                    p.teammatch_url
                ...
                .join('\n\n') +
              '\n\n' +
              'Alle Deine Spiele findest Du übrigens unter https://www.turnier.de/sport/membermatches.aspx?id=' +
              g.problems[0].tournament_id +
              '\n\n' +
              'Viele Grüße\n'
          );
        }
        ...
        g.stb_mail_subject = encodeURIComponent(match_name);
        g.stb_mail_body = encodeURIComponent(
          'Hallo ' +
            ...
            ' ' +
            (g.problems.length > 1
              ? 'wurden ' + g.problems.length + ' mögliche'
              : 'wurde ein möglicher') +
            ' Fehler gefunden:\n\n' +
            g.problems
              .map(p => {
