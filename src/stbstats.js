    ...
    function(season) {
      worker_utils.in_background(worker_fn, season, function(err, wr) {
        ...
        render(req, res, next, 'stbstats_show', {
          ...
          region_filter,
          extended:
            req.query.hasOwnProperty('extended') ||
            req.query.hasOwnProperty('e'),
        });
      });
    }
  );
...
function calc_ms(data, tm, stb_name, now) {
  ...
  if (
    /\(A\)/.test(tm.detailergebnis_user) ||
    tm.detailergebnis_user === tm.ergebnisbestaetigt_user
  ) {
...
function run_calc(season, cb) {
  loader.load_season_data(season, err => {
    ...
    for (const tm of data.teammatches) {
      ...
      const stb_regions = utils.setdefault(
        regions_by_stb,
        stb_name,
        () => new Set()
      );
      ...
      let stb_durations = utils.setdefault(
        durations_by_stb,
        stb_name,
        () => []
      );
      ...
      let region_durations = utils.setdefault(
        durations_by_stb,
        region,
        () => []
      );
