...
function* check_comment(data, pm) {
  ...
  const comment = data.get_comment(pm.teammatchid, text =>
    /krank|verletz|aufgegeben|Aufgabe/i.test(text)
  );
  if (comment) {
    return;
  }
  ...
  for (const tm of data.teammatches) {
    ...
    if (
      ...
      yield {
        ...
        message:
          'Eintrag im Textfeld Spielaufgabe, aber im Detailbericht keine Spiele als aufgegeben gekennzeichnet',
      };
    }
  }
};
