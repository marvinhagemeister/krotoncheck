...
function* get_double_teams(season) {
  const eligible_groups = data_utils.parse_grouplist(
  ...
  for (const team of season.data.teams) {
    ...
    if (['U19', 'Mini'].includes(ltype)) {
      ...
      if (
        !eligible_groups.includes(gid) &&
        !eligible_groups.includes(short_gid)
      ) {
      ...
      if (other_team.clubcode === team.clubcode) {
        if (played_other) {
          const message =
            ...
            ' wird erst am ' +
            utils.ts2destr(tm.ts) +
            ' gespielt, nach ' +
            data_utils.tm_str(played_other) +
            ' am ' +
            utils.ts2destr(played_other.ts) +
            '.';
          yield {
