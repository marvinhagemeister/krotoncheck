...
function* check_fixed_date(player, team, vrl_entry, tm) {
  if (!vrl_entry.fixed_from) {
    const message =
      ...
      team.name +
      ' festgespielt, ' +
      'aber im Eintrag in VRL ' +
      vrl_entry.typeid +
      ' von (' +
      vrl_entry.clubcode +
      ') ' +
      vrl_entry.clubname +
      ' fehlt das "Fest ab"-Datum.';
    yield {
      type: 'vrl',
  ...
  if (!utils.same_day(fixed_from, expected)) {
    const message =
      ...
      ', ' +
      'richtig wäre ' +
      JSON.stringify(utils.ts2dstr(expected));
    yield {
      type: 'vrl',
      teammatch_id: tm.matchid,
      player_id: player.spielerid,
...
function* check_vrl_entry(data, should_fixed, vrl_entry, player) {
  if (!should_fixed.team) {
    if (vrl_entry.fixed_in) {
      ...
      const vrl_team = vrl_entry.teamcode
        ? data.get_team(vrl_entry.teamcode)
        : null;
      const message =
        data_utils.player_str(player) +
        ' steht in VRL ' +
        ...
        ' als ' +
        ' Fest in ' +
        JSON.stringify(vrl_entry.fixed_in) +
        ...
        (vrl_team && vrl_entry.fixed_in === vrl_team.number
          ? ' (vkz3="FIX" beim Festschreiben vergessen?)'
          : '');
      yield {
        type: 'vrl',
        player_id: player.spielerid,
  ...
  if (should_fixed.tm.is_buli) {
    if (vrl_entry.fixed_in) {
      ...
      const message =
        ...
        ' festgespielt (Bundesliga!), ' +
        'aber im Eintrag in ' +
        data.vrl_name(vrl_entry.typeid) +
        ' von (' +
        vrl_entry.clubcode +
        ') ' +
        vrl_entry.clubname +
        ' fehlt ' +
        '[Fest in] = ' +
        JSON.stringify(should_fixed.team.number) +
        ' und [Fest ab] = ' +
        JSON.stringify(utils.ts2dstr(utils.next_day(tm.ts)));
      yield {
        type: 'vrl',
        teammatch_id: should_fixed.tm.matchid,
        player_id: player.spielerid,
...
function* check_round(data, player, matches, is_hr, o19) {
  ...
  for (const pm of matches) {
    ...
    if (
      (match_league_type === 'O19' || match_league_type === 'Bundesliga') !==
      !!o19
    ) {
    ...
    const is_team1 =
      pm.team1spieler1spielerid === player.spielerid ||
      pm.team1spieler2spielerid === player.spielerid;
    ...
    if (tm.is_buli) {
      if (!vrl_entry) {
        ...
        const ve = data.try_get_vrl_entry(
          club_id,
          vrl_type,
          player.spielerid,
          true
    ...
    if (!vrl_entry) {
      ...
      const youth_in_o19 =
        match_league_type === 'O19' &&
        (ve.vkz1 === 'J1' || ve.vkz1 === 'S1' || ve.vkz1 === 'M1');
      if (youth_in_o19) {
    ...
    if (
      should_fixed.team &&
      should_fixed.team.code !== team.code &&
      !vrl_entry.fixed_in
    ) {
      if (
        pm[`flag_umwertung_gegen_team${is_team1 ? 1 : 2}`] ||
      ...
      const message =
        data_utils.player_str(player) +
        ' hat sich am ' +
        should_fixed.tm.spieldatum +
        ' in (' +
        should_fixed.team.code +
        ') ' +
        should_fixed.team.name +
        ' festgespielt, ' +
        'hat aber danach am ' +
        tm.spieldatum +
        ' für (' +
  ...
  if (!vrl_entry && matches.length === 0) {
    ...
    const vrl_type = laws.get_round_vrl_type(
      o19 ? 'O19' : 'U19',
      is_hr,
      player.sex
    );
    vrl_entry = data.try_get_vrl_entry(
      player.clubid,
      vrl_type,
      player.spielerid,
      true
