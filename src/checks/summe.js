  ...
  for (let team = 1; team <= 2; team++) {
    ...
    if (sum !== in_table) {
      ...
      const message =
        ...
        ' ' +
        sum +
        ' Ballwechselpunkte erzielt, aber die angegebene Summe ist ' +
        in_table;
      yield {
  ...
  for (let team_id = 1; team_id <= 2; team_id++) {
    ...
    if (correct_games !== in_table) {
      ...
      const message =
        ...
        ' ' +
        correct_games +
        ' Sätze gewonnen, aber die angegebene Satzsumme ist ' +
        in_table;
      yield {
        teammatch_id: pm.teammatchid,
        match_id: pm.matchid,
