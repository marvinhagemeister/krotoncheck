...
function matching_comment(comments, keywords) {
  return comments.some(comment =>
    keywords.some(kw => comment.nachricht.includes(kw))
  );
}
...
module.exports = function*(season) {
  ...
  for (const tm of data.teammatches) {
    ...
    if (comments.length === 0) {
      const message =
        'Am ' +
        change_line.zeitpunkt +
        ' wurde der Spielort ' +
        ' auf ' +
        loc.name +
        ' geändert, aber kein Kommentar dazu geschrieben';
      yield {
    ...
    if (!matching_comment(comments, keywords)) {
      const message =
        ...
        ' auf ' +
        loc.name +
        ' geändert, aber der zugehörige Kommentar fehlt';
      yield {
        teammatch_id: tm.matchid,
        message,
      };
