...
function is_flagged(pm, team_idx) {
  return (
    pm['flag_umwertung_gegen_team' + team_idx] || pm.flag_keinspiel_keinespieler
  );
}

function is_handled(pm, team_idx) {
  return (
    is_flagged(pm, team_idx) ||
    pm['flag_keinspiel_keinspieler_team' + (3 - team_idx)]
  );
}

function* check_min_count(
  data,
  league_type,
  tm,
  team,
  valid_players_by_gender
) {
  ...
  if (m_count + f_count === 0) {
    if (tm.flag_ok_gegen_team1 || tm.flag_ok_gegen_team2) {
      ...
      yield {
        teammatch_id: tm.matchid,
        message:
          'Keine spielberechtigte SpielerInnen von ' +
          team.name +
          ' aufgestellt (Kampflos falsch eingetragen? Sollte kein Detailergebnis haben)',
      };
    }
    return;
  }
  ...
  if (/^O19-(?:OL|RL|[SN][12]-VL)$/.test(tm.eventname)) {
    if (m_count === 0) {
      yield {
        teammatch_id: tm.matchid,
        message:
          'Keine spielberechtigten Spieler von ' +
          team.name +
          ' aufgestellt (§58.1 SpO)',
      };
    } else if (m_count < 4) {
      ...
      yield {
        teammatch_id: tm.matchid,
        message:
          'Nicht genügend spielberechtigte Spieler von ' +
          team.name +
          ' aufgestellt (§58.1 SpO). Spielberechtigt waren ' +
          names,
      };
    }
    if (f_count === 0) {
      yield {
        teammatch_id: tm.matchid,
        message:
          'Keine spielberechtigten Spielerinnen von ' +
          team.name +
          ' aufgestellt (§58.1 SpO)',
      };
    } else if (f_count < 2) {
      ...
      yield {
        teammatch_id: tm.matchid,
        message:
          'Nicht genügend spielberechtigte Spielerinnen von ' +
          team.name +
          ' aufgestellt (§58.1 SpO). Spielberechtigt war ' +
          names,
      };
    }
    ...
    if (f_count + m_count < 3) {
      const player_ids = new Set([
        ...valid_players_by_gender.M,
        ...valid_players_by_gender.F,
      ]);
      ...
      yield {
        ...
        message:
          'Nicht genügend spielberechtigte SpielerInnen von ' +
          team.name +
          ' aufgestellt (§15.4 JSpO). Spielberechtigt waren ' +
          names,
      };
    }
  } else {
    ...
    if (f_count === 0) {
      yield {
        teammatch_id: tm.matchid,
        message:
          'Keine spielberechtigte Dame von ' +
          team.name +
          ' aufgestellt (§57.5 SpO)',
      };
    }
    ...
    if (
      ...
      const message =
        'Nicht genügend spielberechtigte SpielerInnen von ' +
        team.name +
        ' aufgestellt (§57.4 SpO). ' +
        'Spielberechtigte Herren waren ' +
        names_m +
        ', spielberechtigte Damen ' +
        names_f;
      yield {
        teammatch_id: tm.matchid,
        message,
...
function contains_backup_player(data, tm, players) {
  const backup_players = data.get_matchfield(
    tm,
    'vorgesehene Ersatzspieler (NUR Verbandsliga aufwärts, § 58 SpO)'
  );
  if (!backup_players) {
    return false;
  }

  const notes = data.get_matchfield(
    tm,
    "weitere 'Besondere Vorkommnisse' lt. Original-Spielbericht"
  );
  const resigned = data.get_matchfield(
    tm,
    'Spielaufgabe (Spielstand bei Aufgabe, Grund), Nichtantritt'
  );
  if (!notes && !resigned) {
    return false;
  }
  ...
  for (const p of players) {
    if (
      backup_players.includes(p.name) &&
...
function* check_pm(
  ...
  for (let player_idx = 1; player_idx <= 2; player_idx++) {
    let player_id =
      pm['team' + team_idx + 'spieler' + player_idx + 'spielerid'];
    if (!player_id) {
      continue;
    }
    ...
    if (!ve) {
      if (league_type === 'U19') {
        ...
        const mini_ve = data.get_vrl_entry(
          team.clubcode,
          mini_vrl_type,
          player_id
        );
        ...
        const u19_ve = data.try_get_vrl_entry(
          team.clubcode,
          u19_vrl_type,
          player_id
        );
    ...
    if (ve.fixed_in && (!ve.fixed_from || ve.parsed_fixed_from <= tm.ts)) {
      ...
      if (!laws.is_backup(registered_in, team)) {
        const message =
          ...
          ') ' +
          registered_in.name +
          ', hat aber für die tiefere Mannschaft (' +
          team.code +
          ') ' +
          team.name +
          ' gespielt';
  ...
  for (const discipline in pm_ratings_by_discipline) {
    ...
    for (let i = 0; i < match_ratings.length - 1; i++) {
      ...
      if (laws.is_doubles(discipline)) {
        ...
        if (
          mr1.pm.flag_keinspiel_keinespieler &&
          mr2.pm.flag_keinspiel_keinespieler
        ) {
        ...
        const message =
          ...
          ' ' +
          data_utils.player_name(p1a) +
          ' DVRL #' +
          mr1.ratings[0] +
          '' +
          ' / ' +
          data_utils.player_name(p1b) +
          ' DVRL #' +
          mr1.ratings[1] +
          '.\n' +
          data_utils.match_name(mr2.pm) +
          ' ' +
          data_utils.player_name(p2a) +
          ' DVRL #' +
          mr2.ratings[0] +
          '' +
          ' / ' +
          data_utils.player_name(p2b) +
          ' DVRL #' +
          mr2.ratings[1] +
          '.';
        yield {
        ...
        const message =
          ...
          data_utils.player_str(p1) +
          ' ist VRL #' +
          mr1.ratings[0] +
          ' und hat ' +
          data_utils.match_name(mr1.pm) +
          ' gespielt. ' +
          data_utils.player_str(p2) +
          ' ist VRL #' +
          mr2.ratings[0] +
          ' und hat ' +
          data_utils.match_name(mr2.pm) +
    ...
    for (const pm of dpms) {
      if (problematic.has(pm.matchid)) {
        ...
        if (
          ...
          !pm[`team${3 - team_idx}spieler1spielerid`]
        ) {
          continue; // Opposing teams had no players
        }
        if (
          pm[`flag_keinspiel_keinspieler_team${team_idx}`] &&
          !pm[`team${team_idx}spieler1spielerid`]
        ) {
          continue; // Incorrectly set up team had no players
        }

        const message =
          tm[`team${team_idx}name`] +
          ' hat im ' +
          data_utils.match_name(missing) +
          ' nicht korrekt aufgestellt; damit ' +
          'muss auch das ' +
          data_utils.match_name(pm) +
          ' umgewertet werden (§59.1d SpO)';
        yield {
          match_id: pm.matchid,
          teammatch_id: tm.matchid,
          message,
...
function* check(data, tm) {
  ...
  if (
    problems1.length === 0 &&
    problems2.length === 0 &&
    data.get_stb_note(tm.matchid, text =>
      /^\s*"?\s*Umwertung\s+U(?:14|16)/.test(text)
    ) &&
    !data.get_stb_note(tm.matchid, text =>
      /Um(?:be)?wertung\s+zurück\s*genommen|Rücknahme\s+der\s+Umwertung/.test(
        text
      )
    )
  ) {
    const message = 'Umwertungsgrund kann nicht gefunden werden.';
    yield {
      teammatch_id: tm.matchid,
      message,
    };
