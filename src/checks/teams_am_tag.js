...
function extract_date(tm) {
  const m = /^([0-9]{1,2}\.[0-9]{1,2}\.[0-9]{4}) [0-9]{2}:[0-9]{2}:[0-9]{2}$/.exec(
    tm.spieldatum
  );
  if (!m) {
    throw new Error(
      'Cannot parse date ' +
        JSON.stringify(tm.spieldatum) +
        ' of teammatch ' +
        tm.mathcid +
        '!'
    );
      ...
      for (const player_id of team_players) {
        ...
        if (already_played) {
          if (already_played.team_id !== team_id) {
            ...
            const message =
              data_utils.player_str(p) +
              ' hat am ' +
              date +
              ' für zwei verschiedene Mannschaften gespielt: ' +
              already_played.tm.team1name +
              ' - ' +
              already_played.tm.team2name +
              ' (' +
              already_played.tm.spieldatum +
              ')' +
              ' und ' +
              tm.team1name +
              ' - ' +
