...
function has_late_note(data, tm) {
  return !!data.get_stb_note(tm.matchid, ntext =>
    /[fF](?:04|24|38)/.test(ntext)
  );
}
...
function* check_tm(season, tm) {
  ...
  if (
    !is_olrl &&
    league_type === 'O19' &&
    (original_weekday !== 6 || original_timestr !== '18:00:00')
  ) {
    const message =
      'Verbandsansetzung nicht Samstag 18:00, sondern ' +
      utils.weekday_destr(original_ts) +
      ' ' +
      utils.ts2destr(original_ts) +
      ' (§45.2a SpO)';
    yield {
    ...
    (league_type === 'Mini' || league_type === 'U19') &&
    (original_weekday != 6 || original_timestr != '15:00:00')
  ) {
    const message =
      'Verbandsansetzung nicht Samstag 15:00, sondern ' +
      utils.weekday_destr(original_ts) +
      ' ' +
      utils.ts2destr(original_ts) +
      ' (§45.2b SpO)';
  ...
  const lastdate_str =
    season[
      'lastdate_' + (is_olrl ? 'olrl' : league_type === 'O19' ? 'o19' : 'u19')
    ];
  if (lastdate_str) {
    ...
    if (played > last_ts) {
      const message =
        'Spiel auf ' +
        tm.spieldatum +
        ' verlegt, nach letztem Spieltag ' +
        utils.ts2destr(last_ts) +
        ' (§46.1e SpO)';
      yield {
        teammatch_id: tm.matchid,
        message,
  ...
  const team_entered = tm.mannschaftsergebnis_eintragedatum
    ? utils.parse_date(tm.mannschaftsergebnis_eintragedatum)
    : null;
  const entered = tm.detailergebnis_eintragedatum
    ? utils.parse_date(tm.detailergebnis_eintragedatum)
    : null;
  ...
  if (
    team1.Status === 'Mannschaftsrückzug' ||
    team2.Status === 'Mannschaftsrückzug'
  ) {
    return;
  }
    ...
    if (entered) {
      if (played + REPORT_TEAM_RLOL < entered) {
        const message =
          ...
          tm.spieldatum +
          ', ' +
          'aber erst eingetragen um ' +
          utils.ts2destr(entered) +
  ...
  const report_until = [6, 0].includes(utils.weekday(played))
  ...
  if (entered) {
    if (report_until < entered) {
      const message =
        'Detailergebnis zu spät eingetragen: ' +
        'Spiel am ' +
        utils.weekday_destr(played) +
        ', ' +
        tm.spieldatum +
        ', ' +
        'aber erst eingetragen am ' +
        utils.weekday_destr(entered) +
        ', ' +
        utils.ts2destr(entered);
      yield {
        teammatch_id: tm.matchid,
  ...
  if (
    ...
    const message =
      ...
      ' noch nicht vom StB bearbeitet' +
      ' (Spiel am ' +
      utils.weekday_destr(played) +
      ', ' +
      tm.spieldatum +
      ', Detailergebnis eingetragen am ' +
      utils.weekday_destr(entered) +
      ', ' +
      utils.ts2destr(entered) +
      ')';
