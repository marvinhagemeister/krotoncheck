  ...
  for (const vrl_type of look_in) {
    ...
    if (ve) {
      const message =
        ...
        line.memberid +
        ') ' +
        'steht mit Kennzeichen ' +
        (line.vkz1 || line.jkz1) +
        ' in der ' +
        data.vrl_name(line.typeid) +
        ' von ' +
        '(' +
        clubcode +
        ') ' +
...
function* check_in_youth_team(season, is_hr, line) {
  ...
  if (expect_team === 'J1') {
    if (line.sex === 'M') {
      ...
      throw new Error(
        'Ungültige Geschlechtsangabe: ' + JSON.stringify(line.sex)
      );
    }
    ...
    throw new Error(
      'Ungültiges Kennzeichen für M1/J1-Überprüfung: ' +
        JSON.stringify(expect_team)
    );
  }
  ...
  const tms = utils.uniq(
    pms
      .filter(pm => {
        if (pm.flag_umwertung_gegen_team1) {
          if (
            pm.team1spieler1spielerid === line.memberid ||
            pm.team1spieler2spielerid === line.memberid
          ) {
            return false;
          }
        }
        if (pm.flag_umwertung_gegen_team2) {
          if (
            pm.team2spieler1spielerid === line.memberid ||
            pm.team2spieler2spielerid === line.memberid
          ) {
            return false;
          }
        }
  ...
  const o19_tms = tms.filter(
    tm => data_utils.league_type(tm.staffelcode) === 'O19'
  );
  ...
  if (max_date_str) {
    ...
    if (actual_startdate > max_date) {
      const message =
        ...
        ')' +
        ' in ' +
        season.data.vrl_name(line.typeid) +
        ' ist ab ' +
        line.startdate +
        ' spielberechtigt, aber dieses Datum ist nach Saisonende';
      yield {
        type: 'vrl',
        vrl_typeid: line.typeid,
...
function* check_startend(season, is_hr, vrl_date, line) {
  ...
  if (line.startdate) {
    ...
    if (startdate < vrl_date) {
      const message =
        ...
        line.memberid +
        ')' +
        ' liegt vor Abgabeschluss der ' +
        (is_hr ? 'Hinrunden' : 'Rückrunden') +
        '-VRL' +
  ...
  if (line.enddate) {
    ...
    if (enddate < vrl_date) {
      const message =
        ...
        line.memberid +
        ')' +
        ' darf nicht mehr in der ' +
        (is_hr ? 'Hinrunden' : 'Rückrunden') +
        '-VRL stehen,' +
        ' da ' +
        (line.sex === 'M' ? 'er' : 'sie') +
        ' vor Abgabeschluss (am ' +
        line.enddate +
        ') abgemeldet wurde.';
      yield {
        type: 'vrl',
  ...
  // Enddate required?
  if (
    season.data.spielgemeinschaften &&
    line.clubcode !== line.playerclubcode &&
    !season.data.in_sg(line.clubcode, line.playerclubcode)
  ) {
    if (line.enddate) {
      return;
    }
...
function* check_vrl(season, vrl) {
  ...
  const date_key =
    'vrldate_' + (is_o19 ? 'o19' : 'u19') + '_' + (is_hr ? 'hr' : 'rr');
  ...
  for (const line of vrl.entries) {
    ...
    const position_doubles = line.teampositiondouble
      ? parseInt(line.teampositiondouble)
      : position;
    ...
    if (by_doubles_pos.has(position_doubles)) {
      ...
      const message =
        'Doppelter Eintrag in der Doppel-' +
        data.vrl_name(vrl.typeid) +
        ' von (' +
        vrl.clubcode +
        ') ' +
    ...
    if (
      line.vkz1 &&
      !['U19E', 'SE', 'J1', 'S1', 'M1', 'N'].includes(line.vkz1)
    ) {
      const hint =
        {
    ...
    if (is_o19) {
      ...
      if (m) {
        if (!line.vkz1) {
          // Invalid vkz1 is handled elsewhere
          const message =
            'Jugendkennzeichen (vkz1) fehlt in ' +
            data.vrl_name(vrl.typeid) +
            ' von (' +
            vrl.clubcode +
            ') ' +
        ...
        if ((line.jkz1 === 'U19E' || line.vkz1 === 'U19E') && m[1] == '19') {
          ...
          const message =
            'Falsches Jugendkennzeichen ' +
            JSON.stringify(line.jkz1) +
            ' in ' +
            data.vrl_name(vrl.typeid) +
            ' von (' +
            vrl.clubcode +
    ...
    if (line.teamcode) {
      ...
      if (m) {
        ...
        if (
          ...
          const message =
            ...
            ' ' +
            line.lastname +
            ' hat Mannschaftsnummer ' +
            team_char +
            team_num +
            ', aber vorherige Zeile war ' +
            last_team_char +
            last_team_num;
          yield {
            type: 'vrl',
      ...
      if (last_team_num) {
        // If not, this may be a Bundesliga entry
        const message =
    ...
    if (line.comment) {
      if (/fest/i.test(line.comment) && !line.fixed_in) {
        const message =
          'Kommentar erwähnt Festschreibung (' +
          JSON.stringify(line.comment) +
          '), ' +
